...
install_scripts() {
    ...
    if [[ -f "clawrevops_setup.py" ]]; then
        cp "clawrevops_setup.py" /usr/local/bin/
    else
        curl -fsSL "$REPO_BASE/clawrevops_setup.py" -o /usr/local/bin/clawrevops_setup.py
    fi
...
#!/bin/bash
REPO_BASE="${REPO_BASE}"
curl -fsSL "\$REPO_BASE/clawrevops_setup.py?\$(date +%s)" -o /usr/local/bin/clawrevops_setup.py \
    && chmod +x /usr/local/bin/clawrevops_setup.py \
