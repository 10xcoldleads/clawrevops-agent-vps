...
It can install:
  ...
  4) Claude Code or OpenAI Codex CLI
  5) Hermes Agent or OpenClaw
  6) Agent folders and workspace
...
{Colors.ENDC}""")
  ...
  That's fine for now — but SSH hardening (which disables password
  login) will stay LOCKED until you add an SSH key for '{user}'.

  To add a key later, from YOUR computer run:
...
""")
        ...
        proceed = self.get_user_input(
            "How would you like to authenticate Tailscale?",
        ...
        if proceed == 2:
            ...
            proceed = self.get_user_input(
                ...
                ["Browser login", "Auth key", "Skip Tailscale setup"],
                default_index=0
            )
            if proceed == 2:
...
""")
    ...
    # ── Step: optional SSH hardening (always LAST) ────────────────────────────
